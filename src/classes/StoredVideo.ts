  ...
  implements ITexture, IEmission, IVolume, IPlaylist, IPlayer
{
  ...
  startPlaylist: CallableFunction = () => {
    ...
    if (this.videoTexture && this.playlist.length > 1) {
  ...
  implements ITransform
{
  ...
  applyCustomTransforms: CallableFunction = (
    ...
    if (sdkImagesAreFlipped && isImageTexture && sdkImageFlippedDimension) {
      this.modifiedTransform.rotation[sdkImageFlippedDimension] += 180;
    }

    // TODO - get rid of this flipping system and just readjust the plane UVs
    // if (sdkVideosAreFlipped && isVideoTexture && sdkVideoFlippedDimension) {
